...
MVP1完了後、Playwright基盤と初期ブラウザテストが追加され、作品一覧・本人レビュー一覧のページネーション、退会確認モーダル、レビュー削除モーダルの実ブラウザ操作が自動化されました。以下の「MVP2以降」「手動確認」という判定はMVP1時点の歴史的記録として維持し、現在のPlaywright運用は[Playwrightブラウザテスト運用ガイド](PLAYWRIGHT_TESTING.md)を参照してください。
...
| モーダルのJavaScript実動作・フォーカス移動 | 手動確認 | MVP1時点ではFeatureテストで実ブラウザ操作を保証できないため手動確認とした。後続のPlaywright E2Eで対象モーダルの主要操作を自動化済み |
...
| [ ] | ページネーション | リンクをクリックして正しく遷移する | MVP1時点は手動確認。後続のPlaywright E2Eで作品一覧・本人レビュー一覧を自動化済み |
