...
- アップロードのバリデーションとpublicディスクへの保存を追加する
- 新画像保存、`avatar_path` 更新、旧画像削除の順に処理し、失敗時もDBとファイルの整合性を維持する
- 新画像保存失敗時はDBと旧画像を変更せず、DB更新失敗時は新画像を削除して既存状態を維持し、旧画像削除失敗時はDB更新と新画像表示を成功扱いとする
- 退会時はユーザー固有画像だけを削除対象とし、画像削除失敗やファイル不存在でDBの退会処理を失敗させない
- `public/images/no-image.png` を固定アセットとして追加し、未設定・退会済み・ファイル不存在時の表示へ使用する
- ナビゲーション、アカウント画面、作品詳細のレビュー・返信投稿者、本人レビュー一覧へ円形アイコン表示を追加する
...
保存画像自体のリサイズ、トリミング、WebPへの自動変換、EXIF除去などは本項目に含めず、後続フェーズで対応する。
