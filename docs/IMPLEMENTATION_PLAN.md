...
- [MVP2の実施方針](#mvp2の実施方針)
- [前提](#前提)
- [IssueとPull Requestの関係](#issueとpull-requestの関係)
- [実装フェーズ](#実装フェーズ)
- [Pull Request分割方針](#pull-request分割方針)
  - [PR番号の扱い](#pr番号の扱い)
- [実装順序](#実装順序)
  - [PR1. DBマイグレーション + Seeder](#pr1-dbマイグレーション--seeder)
  - [PR2. Eloquentモデル + リレーション](#pr2-eloquentモデル--リレーション)
  - [PR3. 作品一覧・詳細表示](#pr3-作品一覧詳細表示)
  - [PR4. レビュー表示・レビュー返信表示](#pr4-レビュー表示レビュー返信表示)
  - [PR5. Breeze認証確認](#pr5-breeze認証確認)
  - [PR6. アカウント画面](#pr6-アカウント画面)
  - [PR7. レビュー投稿・削除 + 評価キャッシュ更新](#pr7-レビュー投稿削除--評価キャッシュ更新)
  - [PR8. 本人レビュー一覧 + ページネーション](#pr8-本人レビュー一覧--ページネーション)
  - [PR9. レビュー返信投稿](#pr9-レビュー返信投稿)
  - [PR10. 会員退会](#pr10-会員退会)
  - [PR11. ユーザーアイコンの登録・差し替え・表示](#pr11-ユーザーアイコンの登録差し替え表示)
- [DB実装時の補足方針](#db実装時の補足方針)
  - [初期データ](#初期データ)
  - [unique制約](#unique制約)
  - [index](#index)
  - [外部キー削除方針](#外部キー削除方針)
  - [rating制約](#rating制約)
  - [評価キャッシュ](#評価キャッシュ)
- [Service / FormRequest / Policy 方針](#service--formrequest--policy-方針)
  - [Service](#service)
  - [Form Request](#form-request)
  - [Policy](#policy)
- [Featureテスト候補](#featureテスト候補)
- [PRごとの確認項目](#prごとの確認項目)
  - [共通確認](#共通確認)
  - [Laravel確認](#laravel確認)
  - [DB再構築確認](#db再構築確認)
...
このドキュメントでは、映画レビューアプリ Laravel移植版の実装フェーズ、技術的な依存関係、大まかな実施順序、実装方針を整理する。
...
`docs/FEATURES.md` はMVPごとの機能・基盤のロードマップを整理するドキュメントであり、このドキュメントでは実際にどの順番で実装するかを扱う。
...
MVP2の最新スコープ・優先順位・進捗は、GitHub Milestone「MVP2 - 初回公開版（v1.0.0）」と所属Issue / Pull Requestを正本とする。この文書ではIssueのOpen / Closed状態や完了日を逐次管理しない。

現時点の大まかな依存関係は次のとおりである。
...
これは固定の進捗表ではなく、管理者画面と認証・認可を整えた後にTMDB連携と公開準備へ進むための依存関係を示す。TMDB APIの認証方式、endpoint、取得・保存項目、エラー処理、重複登録防止方式は技術調査後に決定し、この文書では確定しない。

## 前提

以下はMVP1の初期移植時に、既存スクラッチ版の最低限機能をLaravel 10へ移植した際の前提である。現在の技術baselineへ機械的に読み替えない。
...
| Phase 3 | 会員機能 | Breeze確認、アカウント管理、レビュー投稿、削除、返信、退会 |
...
- 計画時点では会員登録画面・ログイン画面のUI調整を後続PRへ分離し、Issue #16で対応済み
...
- 計画時点では会員登録画面・ログイン画面がBreezeデフォルトUIをベースとしていたこと。映画レビューアプリ向けUI調整はIssue #16で完了していること

補足:

- 認証処理はLaravel Breeze標準を利用する
- 映画レビューアプリ向けの認証画面UI調整はIssue #16で対応済み

### PR6. アカウント画面

ログイン必須画面の実装に慣れるため、アカウント画面を先に実装する。

主な作業内容:

- Breeze標準のアカウント画面を確認する
- 必要に応じてプロフィール項目を調整する
- `profile` を更新できるようにする
- `avatar_path` を利用するユーザーアイコン機能は専用実装へ分離する
...
- アカウント画面にBreeze形式の会員退会確認モーダルを設置する
...
### PR11. ユーザーアイコンの登録・差し替え・表示

既存のアカウント画面の編集フォーム、レビュー表示、本人レビュー一覧、会員退会へユーザーアイコン機能を統合する。新しいルートと `users` テーブルの追加マイグレーションは作成しない。

主な作業内容:

- アップロードのバリデーション、保存前プレビュー、publicディスクへの保存を追加する
- 新画像保存、`avatar_path` 更新、旧画像削除の順に処理し、失敗時もDBとファイルの整合性を維持する
- 新画像保存失敗時はDBと旧画像を変更せず、DB更新失敗時は新画像を補償削除し、例外発生時は元例外を再送出する
- 旧画像削除失敗時は報告し、DB更新と新画像表示を成功扱いとする
- 差し替え・退会時の削除対象を `avatars/` 配下のユーザー固有画像に限定し、退会時は画像削除失敗やファイル不存在でDBの退会処理を失敗させない
- `public/images/no-image.png` を固定アセットとして追加し、未設定・退会済み・ファイル不存在・許可ディレクトリ外のパスの表示へ使用する
- PC・モバイルナビゲーション、アカウント画面、作品詳細のレビュー投稿者・レビュー返信投稿者、本人レビュー一覧へ円形アイコン表示を追加する
- 既存のEager Loadingを利用し、アイコン表示によるN+1を発生させない
- Feature Testを追加し、実装後にREADME・関連docsとの整合を確認する

確認項目:

- JPEG、PNG、WebPのみを2MB以内で受け付け、ファイル内容に基づいて形式を検証すること
- GIF、SVG、その他の許可外形式および2MBを超える画像を拒否すること
- 新画像未選択時、保存失敗時、DB更新失敗時、旧画像削除失敗時の結果が確定仕様どおりであること
- 未設定・退会済み・ファイル不存在時にNo Image画像へフォールバックすること
- 他人のアイコンを変更できず、画像単独削除と「No Imageへ戻す」機能がないこと
- プロジェクトの開発フローに従って品質確認とレビューを行うこと

保存画像自体のリサイズ、トリミング、WebPへの自動変換、EXIF除去などは本項目に含めず、後続のIssue #61で対応する。
...
- 会員退会後もレビュー返信本文が残る
- JPEG、PNG、WebPのユーザーアイコンを登録・差し替えできる
- ユーザーアイコン未設定時、退会済み、ファイル不存在時にNo Image画像が表示される
- ユーザーアイコン更新・退会時にDBとファイルの整合性が維持される
- 他ユーザーのアイコンを変更できない
- 他ユーザーのレビューを削除できない
