...
9. 差分を確認する
10. GitHub Actionsの品質チェックがすべて成功していることを確認する
11. 運用中のマージ先ブランチへマージする
    - MVP1では作業ブランチから`main`へマージする
    - MVP2以降は作業ブランチから`develop`へマージし、`develop`から`main`へ反映する
12. 不要になったリモートブランチを削除する
13. マージ先のローカルブランチを最新化する
14. 不要になったローカルブランチを削除する
