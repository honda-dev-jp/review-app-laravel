...
このドキュメントでは、映画レビューアプリ Laravel移植版におけるセキュリティ方針を整理する。MVP1初期移植時の方針を基礎とし、MVP2で追加したメール認証、開発基盤、Playwright E2E等の現在の方針も扱う。
...
- 認証が必要な操作は `auth` ミドルウェアで保護する
- メール認証が必要な操作は `verified` ミドルウェアで保護する
...
ログイン必須画面・操作には `auth` ミドルウェアを設定する。レビュー・評価投稿とレビュー返信投稿は、メール認証済みユーザーに限定するため `verified` ミドルウェアも設定する。
...
管理者も既存の `App\Models\User`、`users` テーブル、Laravel Breeze、`web` Guard、`users` Providerを使用して認証する。管理者専用のModel、Guard、Provider、ログイン画面、パスワードリセットは設けない。

認証主体は一般ユーザーと共通とし、`users.role` を管理者権限の認可に使用する。管理者用に認証主体やGuardを分離しない。

### メール認証

メール認証はLaravel BreezeおよびLaravel標準機能を使用し、独自OTP・独自認証コード方式は使用しない。

- `User` モデルで `Illuminate\Contracts\Auth\MustVerifyEmail` を実装する
- 会員登録時の `Registered` イベントにより `VerifyEmail` 通知を送信する
- 認証URLはLaravel標準の署名付きURLを使用する
- 認証URL処理は `signed` ミドルウェアで保護する
- 認証案内、認証URL、認証メール再送は `auth` ミドルウェア配下とする
- 認証URLと認証メール再送は `throttle` ミドルウェアで制限する
- レビュー・評価投稿とレビュー返信投稿は `verified` ミドルウェアで保護する
- 認証リンク有効期限はLaravel標準設定を使用する
- ローカル開発では、MVP1で導入済みのMailpitで認証メール本文と認証URLを確認する

会員登録後は未認証状態でログインし、作品一覧画面へ遷移する。認証が必要な操作へアクセスした場合に、Laravel標準の `verified` ミドルウェアにより認証案内画面へ遷移する。

メールアドレスを変更した場合は `email_verified_at` を `null` に戻し、再認証を必要とする。
...
| 操作 | 認可方針 |
|---|---|
| レビュー・評価投稿 | メール認証済みユーザーのみ投稿可能。1ユーザーにつき1作品1件まで |
| レビュー削除 | 自分のレビューのみ削除可能 |
| 本人レビュー一覧 | 自分のレビューのみ表示 |
| アカウント情報の編集 | 自分のプロフィールのみ編集 |
| 会員退会 | 自分のアカウントのみ退会 |
| レビュー返信投稿 | メール認証済みユーザーのみ投稿可能 |
| 管理画面ダッシュボード | `users.role` が `admin` のユーザーのみ利用可能 |
...
管理画面のように特定モデルへ紐づかないアクセス権限は、`AuthServiceProvider` で定義したGate `access-admin-panel` に集約する。Gateは認証済みユーザーの `role` が `admin` と完全に一致する場合だけ許可し、`user` および未知のroleは許可しない。

`/admin` は `auth` の後に `can:access-admin-panel` ミドルウェアで保護する。未ログインユーザーはログイン画面へ誘導し、認証済みの非adminユーザーは403で拒否する。管理画面の利用にメール認証は要求しない。

### 管理者権限の付与とrole保護

管理者専用の登録UI、admin昇格route、role管理UIは設けない。管理者権限は、通常会員として作成されたユーザーに対し、アプリケーション外の信頼されたDB管理手段を用いて運用者が `role=admin` を付与する。

通常会員登録およびプロフィール更新から `role` を変更できない状態を維持する。ユーザー入力から権限値を直接更新しない共通方針は「10. Mass Assignment対策」に従う。
...
3. メール認証必須ルートに `verified` ミドルウェアを設定する
4. レビュー削除など本人確認が必要な操作にPolicyを検討する
5. すべてのフォームに `@csrf` を設定する
6. Blade出力は `{{ }}` を基本にする
7. DB操作はEloquentまたはクエリビルダを使う
8. ファイルアップロードはランダム名・MIMEチェック・サイズ制限を行う
9. `.env` をGit管理しない
10. Composer / npm依存関係の差分を確認する
11. PR前にテスト・整形・静的解析を実行する
...
TMDB API連携はMVP2で対応する。利用条件・クレジット要件とAPI技術仕様を分離して調査し、認証方式、endpoint、取得・保存項目、timeout、retry等は後続の技術調査で決定する。
...
以下は未実装または後続設計で扱う。MVP2対象とMVP2対象外の将来候補の区分は[機能一覧](FEATURES.md)を参照する。
