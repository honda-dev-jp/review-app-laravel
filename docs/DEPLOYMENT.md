...
デプロイ元は、同期Pull Requestによって`develop`から`main`へ反映済みの公開・リリース可能な変更とする。Issue、ブランチ、同期Pull Request、`main`と`develop`の同期手順は[GitHub開発運用ガイド](GITHUB_WORKFLOW.md)を参照し、この文書では重複して扱わない。
...
初期移植フェーズでは、パスワードリセット以外のメール通知（退会完了メール、お問い合わせメール、メールアドレス確認など）は実装対象外とする。

パスワードリセットメールはMVP1の対象だが、本番SMTP設定は未対応・未検証である。本番環境でパスワードリセットを利用するには、MAIL設定を確定する必要がある。
