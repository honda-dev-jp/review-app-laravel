...
| 本番SMTP設定 | 未検証 | パスワードリセットメールの送信に必要 |
...
パスワードリセットメールを本番環境で送信する場合、および後続フェーズでお問い合わせフォームを実装する場合は、メール送信設定を本番用 `.env` へ追加する。
...
MVP2でメール利用機能を追加する場合も、同じ本番メール方針に従う。

パスワードリセットは実装済みだが、本番SMTP設定は未対応・未検証である。本番環境でメール送信機能を利用するには、MAIL設定を確定する必要がある。
