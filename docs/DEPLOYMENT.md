...
- [1. ドキュメントの位置づけ](#1-ドキュメントの位置づけ)
- [2. 現時点の検証状況](#2-現時点の検証状況)
- [3. デプロイ前提](#3-デプロイ前提)
- [4. XServer運用方針](#4-xserver運用方針)
- [5. 公開ディレクトリ方針](#5-公開ディレクトリ方針)
- [6. 本番反映前の確認（ローカル環境）](#6-本番反映前の確認ローカル環境)
- [7. Composer依存関係](#7-composer依存関係)
- [8. npm / Viteビルド](#8-npm--viteビルド)
- [9. 本番用 `.env` 方針](#9-本番用-env-方針)
- [10. APP_KEY 方針](#10-app_key-方針)
- [11. マイグレーション方針](#11-マイグレーション方針)
- [12. storageと画像](#12-storageと画像)
- [13. キャッシュ方針](#13-キャッシュ方針)
- [14. エラー表示](#14-エラー表示)
- [15. デプロイ手順の暫定案](#15-デプロイ手順の暫定案)
- [16. デプロイ後確認](#16-デプロイ後確認)
- [17. 危険操作として保留する項目](#17-危険操作として保留する項目)
- [18. 現時点で実施しないこと](#18-現時点で実施しないこと)
- [19. 今後検討すること](#19-今後検討すること)
- [20. 次に検証する順番](#20-次に検証する順番)

---

## 1. ドキュメントの位置づけ

このドキュメントは、映画レビューアプリ Laravel移植版を XServer のサブドメインへ安全にデプロイするための方針を整理する。

デプロイ元は、同期Pull Requestによって`develop`から`main`へ反映済みの**公開・リリース可能な変更**とする。
Issue、ブランチ、同期Pull Request、`main`と`develop`の同期手順は
[GitHub開発運用ガイド](GITHUB_WORKFLOW.md)
を参照し、この文書では重複して扱わない。

Laravelのメジャーアップグレードは1 majorずつ`develop`上で確認する。
アップグレード途中の中継baselineは、そのversionが動作していても自動的に本番反映対象とはしない。

Laravelメジャーアップグレードの標準手順は
[Laravelメジャーアップグレードガイド](LARAVEL_UPGRADE_GUIDE.md)、
実際に行ったアップグレードと判断の履歴は
...
| 区分 | 意味 |
|---|---|
| 確定 | プロジェクト方針として確定している内容 |
| 確認済み | 実環境・実コマンドなどで確認済みの内容 |
...
このドキュメントでは、
**開発中のLaravel版**と**本番反映対象のLaravel版**を区別する。

`develop`や作業ブランチで動作確認済みであっても、
サポート状況・Security Advisory・Issueのスコープ上、本番へ出さないversionが存在するためである。

今後、LaravelアップグレードおよびXServer上の検証結果に応じて各項目の状態を更新する。
...
| Mailpitをローカル開発で使用する | 確定 | ローカルのメール確認用 |
| 本番SMTP設定 | 未検証 | パスワードリセットメールとメール認証メールの送信に必要 |
...
| Web実行時PHPバージョン | 確認済み | PHP 8.4.20 を確認済み |
| SSH接続 | 確認済み | WSL Ubuntuから接続確認済み |
| SSH接続ポート | 確認済み | `10022` |
| SSH認証方式 | 確認済み | 公開鍵認証 |
| SSH上の通常 `php` | 確認済み | 現在は`~/bin/php`経由でPHP 8.3.30 |
| SSH上のPHP 8.3 | 確認済み | `/usr/bin/php8.3` でPHP 8.3.30 |
| SSH上のPHP 8.4 | 確認済み | `/usr/bin/php8.4` でPHP 8.4.20 |
...
- mainブランチが最新であること
- `main`に含まれるLaravel版が、このドキュメントで定めた本番反映対象版であること
- Laravelアップグレード途中の中継baselineが誤って`main`へ含まれていないこと
- デプロイ対象が同期Pull Requestでmainブランチへ反映済みであること
- 未コミット変更がないこと
- `.env` をコミットしていないこと
- `composer.lock` があること
...
Laravel Telescope は開発・デバッグ用途のツールであり、現時点では導入しない。
...
Security Blockingや既知脆弱性を残す場合の方針は
[セキュリティ方針](SECURITY.md#15-composer依存関係の注意)
を参照する。

Laravel本体のメジャーアップグレード手順は
[Laravelメジャーアップグレードガイド](LARAVEL_UPGRADE_GUIDE.md)
を参照する。
...
本番反映では、ローカル環境で本番ビルドを行い、生成されたビルド成果物を本番環境へ反映する。
...
パスワードリセットメールとメール認証メールを本番環境で送信する場合、および後続フェーズでお問い合わせフォームを実装する場合は、メール送信設定を本番用 `.env` へ追加する。
...
本番でメールを利用する機能は、各対応Issueの実装状況に合わせて本番SMTP設定を確定する。
MVP2で追加したメール認証も、同じ本番メール方針に従う。

パスワードリセットとメール認証は実装済みだが、本番SMTP設定は未対応・未検証である。本番環境でメール送信機能を利用するには、MAIL設定を確定する必要がある。

本番SMTP未設定のままメール認証を有効にすると、新規会員が認証メールを受信できず、レビュー投稿とレビュー返信投稿を利用できない。

ローカル開発環境ではMailpitを使用してメール内容を確認する。Mailpitは本番環境では使用しない。
...
- ログイン
- ログアウト
- パスワードリセット（本番SMTP設定後に確認する）
- メール認証（本番SMTP設定後に確認する）
- 認証メール再送（本番SMTP設定後に確認する）
- アカウント管理
- 本人のレビュー一覧表示
- レビュー・評価投稿
- レビュー削除
...
- Laravel 11中継baselineの`main`同期
- Laravel 11中継baselineのXServerデプロイ
- 本番反映対象Laravel版が確定する前の本番デプロイ
- 自動デプロイの構築
...
Laravelアップグレード途中の中継版を本番へ出さない判断の詳細は、
[Laravelメジャーアップグレード実施履歴](LARAVEL_UPGRADE_HISTORY.md)
を参照する。
