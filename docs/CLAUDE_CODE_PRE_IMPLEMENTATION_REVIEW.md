...
2. メール認証必須機能では `verified` middlewareの対象が `docs/ROUTES.md`、要件、現行実装と一致しているか
3. `verified` の適用漏れと過剰適用の両方を検出できるか
4. 未ログイン、ログイン済み・メール未認証、認可違反を区別しているか
5. Policy、Gate、Controllerの認可責務が明確か
6. 本人以外のデータを操作できるIDORが起きないか
7. Form RequestまたはLaravel標準バリデーションを利用できるか
8. CSRF保護を維持しているか
9. Bladeの自動エスケープを維持しているか
10. Mass Assignment対策が明確か
11. 秘密情報や個人情報を扱わない設計になっているか
...
1. 正常系が定義されているか
2. 未ログイン、メール未認証、認可違反、存在しないデータの異常系が定義されているか
...
- 登録後の画面遷移と既存ルートへの影響
- 未ログインユーザーとメール未認証ユーザーのアクセス制御を区別できているか
