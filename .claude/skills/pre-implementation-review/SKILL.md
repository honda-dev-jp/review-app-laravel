...
- `gh issue create`、`edit`、`comment`、`close`などの変更操作、bare `gh api`、`gh auth token`、`gh auth status --show-token`は実行しません。Issue #89のGlobal Advisoriesは権限設計書の専用helperだけを使用します。
...
- 外部通信は、前節の読み取り専用`gh issue view`と`gh issue list`、権限設計書の有限host/pathに限定したWebFetch、および§14.4・§14.5のMVP2専用GitHub経路だけを例外とします。
