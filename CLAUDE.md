...
このリポジトリは、PHPスクラッチMVCで作成した映画レビューアプリをLaravelへ移植し、サポート対象版へ段階的にアップグレードする学習・ポートフォリオ用プロジェクトです。
...
- PHP 8.4.24（Issue #130の作業ブランチ上で確認した更新後baseline候補。PR未作成、`develop`・`main`・本番環境へ未反映）
