...
- bare `gh api`、`gh auth token`、`gh auth status --show-token`を実行しないでください。Issue #89のGlobal AdvisoriesとIssue #90のDependabot alertsは、設計書のrepository相対canonical helperだけを使用してください。
...
Global Security Advisories、現行CI ActionのRelease/Release-linked Tag、現在のrepositoryのDependabot alertsは、[Claude Code権限設計](docs/CLAUDE_CODE_PERMISSION_DESIGN.md) §14.4〜§14.6のcanonical形だけを使用できます。
...
- Release asset、source archive、Releaseに紐づかないTag、任意repository、Actions runは参照しないでください。Dependabot alertの変更、影響分析、package更新は行わないでください。
